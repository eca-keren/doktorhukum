<section class="w-full">
    <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-bold tracking-tight">{{ __('News Management') }}</h1>
            <p class="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
                {{ __('Manage all news articles') }}
            </p>
        </div>
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-neutral-600 dark:text-neutral-400">{{ __('No news found') }}</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    <!-- Modal -->
    <flux:modal name="news-modal" wire:ignore.self class="max-w-2xl">
        <form wire:submit="saveNews" class="space-y-6">
            <div>
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="4"
                    class="w-full border rounded-lg block disabled:shadow-none dark:shadow-none appearance-none text-base sm:text-sm py-2 px-3 bg-white dark:bg-white/10 dark:disabled:bg-white/[7%] text-zinc-700 disabled:text-zinc-500 placeholder-zinc-400 disabled:placeholder-zinc-400/70 dark:text-zinc-300 dark:disabled:text-zinc-400 dark:placeholder-zinc-400 dark:disabled:placeholder-zinc-500 shadow-xs border-zinc-200 border-b-zinc-300/80 disabled:border-b-zinc-200 dark:border-white/10 dark:disabled:border-white/5"
                    placeholder="{{ __('Enter news description...') }}"
                    required
                ></textarea>
                @error('description')
    ...
    @script
        <script>
            ...
            Livewire.on('news-created', () => {
                $wire.dispatch('notify', { message: 'News created successfully', type: 'success' });
            });

            Livewire.on('news-updated', () => {
                $wire.dispatch('notify', { message: 'News updated successfully', type: 'success' });
            });

            Livewire.on('news-deleted', () => {
                $wire.dispatch('notify', { message: 'News deleted successfully', type: 'success' });
            });
        </script>
    @endscript
</section>
